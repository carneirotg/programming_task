defmodule ProgrammingTask.ElasticHandler do
  ...
  def handle_cast({:put, mapping}, config) do
    Elastix.Mapping.put(config.elastic_url, config.elastic_index_name, config.elastic_doc_type, mapping)

    search_in = [config.elastic_doc_type]
    IO.inspect(Elastix.Search.search(config.elastic_url, config.elastic_index_name, search_in, %{}))
    {:noreply, config}
  end
